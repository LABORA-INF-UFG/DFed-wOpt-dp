...
2
        ...
        for idx in range(len(row_ind)):
            ...
            if k_power < 0:
                ...
                pot_val = self.cs.tm.user_power[k_power]
                assigned_users_list.append(selected_clients[idx])
                _rb_allocation.append(j_rb)
                _user_power_allocation.append(k_power)
                print(f"User [{i_user}]: {selected_clients[idx]} -> (RB={j_rb}, Power={pot_val:.4f}), cost(q)={cost_val:.6f}, distance: {self.cs.tm.user_distance[selected_clients[idx]]}")
